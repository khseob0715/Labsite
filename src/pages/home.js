...
function Home() {
    ...
    const VisitorCounter = () => {
        ...
        useEffect(() => {
            // counter.dev에서 사용할 고유한 네임스페이스입니다. 다른 사이트와 겹치지 않게 수정해주세요.
            const workspace = 'dxplab-ku-website'; 
            const totalCounterName = 'total';
            // 매일 다른 카운터 이름을 생성합니다 (예: daily-2026-04-23)
            const todayCounterName = `daily-${new Date().toISOString().slice(0, 10)}`;
    
            const fetchCounts = async () => {
                try {
                    // '/up'을 붙여 요청하면 카운트를 1 증가시키고 현재 값을 반환합니다.
                    const totalPromise = fetch(`https://counter.dev/api/${workspace}/${totalCounterName}/up`);
                    const todayPromise = fetch(`https://counter.dev/api/${workspace}/${todayCounterName}/up`);
    
                    const [totalRes, todayRes] = await Promise.all([totalPromise, todayPromise]);
    
                    if (!totalRes.ok || !todayRes.ok) {
                    ...
                    setTotalCount(totalData.count.toLocaleString());
                    setTodayCount(todayData.count.toLocaleString());
        ...
        return (
            <div className="position-fixed shadow rounded-pill bg-white d-flex align-items-center px-3 py-2 lift" 
                 style={{ bottom: '30px', right: '30px', zIndex: 1040, border: '1px solid rgba(139, 0, 41, 0.2)' }}>
                <FeatherIcon icon="bar-chart-2" style={{ width: 16, height: 16, color: '#8b0029', marginRight: '10px' }} />
                <div className="d-flex align-items-center me-3">
                    <span className="fw-bold" style={{ fontSize: '0.8rem', color: '#8b0029' }}>Today</span>
                    <span className="ms-2 fw-bolder text-dark" style={{ fontSize: '0.85rem' }}>{todayCount}</span>
                </div>
                <div className="d-flex align-items-center">
                    <span className="fw-bold" style={{ fontSize: '0.8rem', color: '#8b0029' }}>Total</span>
                    <span className="ms-2 fw-bolder text-dark" style={{ fontSize: '0.85rem' }}>{totalCount}</span>
                </div>
            </div>
        );
    };
